...
### Ideia 7: "Casos como Prova de Qualidade" — 8 casos reais com scoring quantitativo como portfólio do agente

**Descrição:** Nenhum concorrente publica casos reais gerados pela própria ferramenta COM avaliação quantitativa independente (critique score). Os 8 casos do kit (Lumen 4.7/5, Norte 4.6/5, +6) são a prova de que o sistema funciona de ponta a ponta. Posicionar como "não acredite em nós — veja o que o kit constrói e a nota que recebeu".
...
| Design Kit: 8 casos em docs/casos/. Lumen: 2 critiques (v1→v2, 4.7/5). Norte: critique completo (4.6/5). Ponto Final: 10 artefatos cobrindo ciclo completo. Aurora: anti-slop check (14/14). | docs/casos/ (8 diretórios) + avaliacao-inicial.md §1.6 | ✅ Validado |
